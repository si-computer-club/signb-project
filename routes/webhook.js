...
parameters: ${JSON.stringify(agent.parameters)}`);
    ...
    if (reqdata.source == 'line') {
      ...
      intentMap.set('confirm age - yes', intent['confirm age - yes'](agent, userId));
      intentMap.set('confirm age - no', intent['confirm age - no'](agent, userId));
      intentMap.set('profile', intent.profile(agent, userId));
      intentMap.set('name', intent.name(agent, userId));
      intentMap.set('menstruation (y/n)', intent.askMenses(agent, userId));
      intentMap.set('menstruation (y/n) - no', intent.menses(agent, userId, 0));
      intentMap.set('menstruation - light', intent.menses(agent, userId, 1));
